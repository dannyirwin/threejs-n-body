...
(function (THREE) {
  ...
  THREE.Octree = function (parameters) {
    ...
    this.depthMax = isNumber(parameters.depthMax)
      ? parameters.depthMax
      : Infinity;
    this.objectsThreshold = isNumber(parameters.objectsThreshold)
      ? parameters.objectsThreshold
      : 8;
    this.overlapPct = isNumber(parameters.overlapPct)
      ? parameters.overlapPct
      : 0.15;
    this.undeferred = parameters.undeferred || false;

    this.root =
      parameters.root instanceof THREE.OctreeNode
        ? parameters.root
  ...
  THREE.Octree.prototype = {
    ...
    remove: function (object) {
      ...
      if (this.objectsMap[object.uuid]) {
        ...
        index = indexOfPropertyWithValue(
          this.objectsDeferred,
          'object',
          object
        );
    ...
    rebuild: function () {
      ...
      for (i = 0, l = this.objectsData.length; i < l; i++) {
        ...
        if (
          node instanceof THREE.OctreeNode &&
  ...
  THREE.OctreeNode = function (parameters) {
    ...
    this.position =
      parameters.position instanceof THREE.Vector3
        ? parameters.position
        : new THREE.Vector3();
  ...
  THREE.OctreeNode.prototype = {
    ...
    expand: function (objects, octants) {
      ...
      if (this.tree.root.getDepthEnd() < this.tree.depthMax) {
        ...
        if (objectsExpand.length > 0) {
          ...
          indexOctant = this.getOctantIndexFromPosition(
            octantX,
            octantY,
            octantZ
          );
          indexOctantInverse = this.getOctantIndexFromPosition(
            -octantX,
            -octantY,
            -octantZ
          );
    ...
    checkMerge: function () {
      ...
      while (
        nodeParent.parent instanceof THREE.OctreeNode &&
    ...
    checkContract: function () {
      ...
      if (this.nodesIndices.length > 0) {
        ...
        for (i = 0, l = this.nodesIndices.length; i < l; i++) {
          ...
          if (
            nodeHeaviest instanceof THREE.OctreeNode === false ||
        ...
        if (
          outsideHeaviestObjectsCount < this.tree.objectsThreshold &&
  ...
  THREE.Raycaster.prototype.intersectOctreeObjects = function (
    ...
    for (i = 0, il = objects.length; i < il; i++) {
      intersects = intersects.concat(
        this.intersectOctreeObject(objects[i], recursive)
      );
    }
